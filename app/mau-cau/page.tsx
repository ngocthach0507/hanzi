...
const patterns = [
  { id: 1, structure: 'A 是 B', name: 'Mẫu câu "Là"', grammar: 'Dùng để định nghĩa, giới thiệu hoặc khẳng định sự vật.', sample: '我是学生。', pinyin: 'Wǒ shì xuésheng.', vi: 'Tôi là học sinh.', isFree: true },
  { id: 2, structure: 'A 有 B', name: 'Mẫu câu "Có"', grammar: 'Dùng để chỉ sự sở hữu hoặc sự tồn tại.', sample: '我有三本书。', pinyin: 'Wǒ yǒu sān běn shū.', vi: 'Tôi có 3 quyển sách.', isFree: true },
    ...
    <div className="py-12 px-4 md:px-8 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {patterns.map((p) => {
            const canAccess = p.isFree || isPremium;

            return (
              <div 
                key={p.id} 
                className={`bg-white rounded-[40px] border border-gray-100 p-8 flex flex-col group relative overflow-hidden shadow-sm transition-all duration-300 ${
              ...
              >
                ...
                <div className="flex items-center justify-between mb-8">
                   <div className={`p-3 rounded-2xl transition-all shadow-sm ${canAccess ? 'bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white' : 'bg-gray-100 text-gray-300'}`}>
                      <Quote size={24} />
                   </div>
                   <button className={`transition-colors ${canAccess ? 'text-gray-200 hover:text-yellow-400' : 'text-gray-100'}`}>
                      <Star size={20} />
                   </button>
                </div>

                <div className="mb-6">
                   <div className={`text-3xl font-black mb-2 font-serif ${canAccess ? 'text-gray-900' : 'text-gray-300'}`}>{p.structure}</div>
                ...
                <div className="p-5 bg-gray-50 rounded-2xl mb-8 flex-1 border-l-4 border-blue-400">
                   ...
                   <p className="text-xs text-gray-600 leading-relaxed font-medium">{canAccess ? p.grammar : 'Nội dung chi tiết dành cho thành viên Premium...'}</p>
                </div>
                ...
                <div className="space-y-4">
                   ...
                   <div className="bg-white border border-gray-100 p-5 rounded-2xl shadow-sm relative overflow-hidden group/ex">
                      <div className={`text-xl font-black mb-1 ${canAccess ? 'text-gray-800' : 'text-gray-300'}`}>{p.sample}</div>
                      <div className="text-[10px] font-bold text-gray-400 mb-2 italic uppercase">{p.pinyin}</div>
                      <div className={`text-sm ${canAccess ? 'text-gray-600' : 'text-gray-300'}`}>{p.vi}</div>
                      {canAccess && (
                ...
                {canAccess ? (
                  ...
                  <div className="mt-8 flex items-center justify-between text-xs font-black uppercase tracking-[0.2em] text-gray-300">
                     🔒 Premium Content <Lock size={16} />
                  </div>
                )}
              </div>
            );
        ...
        {!isPremium && (
          <div className="mt-16 gradient-premium rounded-[50px] p-12 text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl shadow-orange-200 relative overflow-hidden">
             ...
             <Link href="/nang-cap" className="relative z-10 whitespace-nowrap bg-white text-[#D85A30] px-12 py-6 rounded-[24px] font-black text-2xl hover:scale-110 transition-transform shadow-2xl shadow-black/20 active:scale-95">
                👑 Đăng ký Premium
             </Link>
