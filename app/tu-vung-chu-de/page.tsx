...
import { LayoutGrid, ArrowRight, ChevronRight, Star, Clock, BookOpen } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {topics.map((topic) => (
            <Link 
              key={topic.id} 
              href={`/tu-vung-chu-de/${topic.slug}`}
              className="bg-white rounded-3xl border border-gray-100 p-6 card-hover flex flex-col group"
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#D85A30] transition-colors">{topic.name}</h3>
                  {topic.isFree && <span className="bg-green-100 text-green-600 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Free</span>}
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-50 flex items-center justify-between text-[#D85A30] font-bold text-sm">
                Bắt đầu học
