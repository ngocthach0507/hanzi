...
import { X, User, LayoutDashboard, Sparkles, ArrowDown } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100">
        {/* Nút đóng */}
        <button 
          onClick={() => { setIsOpen(false); localStorage.setItem('hasSeenWelcomeGuide', 'true'); }}
        ...
        <div className="p-10 pt-8 text-center">
          ...
          <div className="bg-gray-50 rounded-3xl p-6 mb-8 border border-gray-100 relative group">
             ...
             <div className="absolute -top-10 right-4 animate-bounce hidden md:block">
                <div className="flex flex-col items-center">
                   <span className="text-[10px] font-black text-red-500 bg-red-50 px-2 py-1 rounded-lg border border-red-100 mb-1 shadow-sm">NÓ Ở ĐÂY NÈ!</span>
             ...
             <div className="flex items-start gap-4 text-left">
                ...
                <div>
                   <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 text-[10px]">Hướng dẫn nhanh</p>
                   <p className="text-sm font-bold text-gray-700 leading-snug">
                     Bấm vào <span className="text-red-600">Ảnh đại diện</span> ở góc trên bên phải &gt; Chọn <span className="text-blue-600 flex items-center gap-1 inline-flex">Hồ sơ cá nhân <LayoutDashboard size={14} /></span> bất cứ lúc nào nhé!
                   </p>
                </div>
             </div>
          </div>
          ...
          <button
            onClick={handleClose}
            className="w-full py-5 rounded-2xl bg-gray-900 text-white font-black text-sm uppercase tracking-widest hover:bg-black transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2"
          >
            KHÁM PHÁ NGAY <LayoutDashboard size={18} />
          </button>
        </div>
      </div>
    </div>
